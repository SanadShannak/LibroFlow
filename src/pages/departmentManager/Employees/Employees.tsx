...
const DepartmentManagerEmployees = () => {
  const extendedEmployees = [
    ...initialEmployees,
    { id: 4, name: "David Lee", email: "david.lee@libroflow.com", role: "Cataloger", branch: "Jabal Amman", savedBy: "Admin" },
    { id: 5, name: "Emma Wilson", email: "emma.wilson@libroflow.com", role: "Assistant", branch: "Jabal Amman", savedBy: "Admin" },
    { id: 6, name: "Frank Miller", email: "frank.miller@libroflow.com", role: "Librarian", branch: "Jabal Amman", savedBy: "Admin" },
    { id: 7, name: "Grace Taylor", email: "grace.taylor@libroflow.com", role: "Clerk", branch: "Jabal Amman", savedBy: "Admin" },
  ];
  const [employees, setEmployees] = useState<Employee[]>(extendedEmployees);
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedEmployee, setSelectedEmployee] = useState<Employee | null>(null);
  const [roleFilter, setRoleFilter] = useState<string | null>(null);
  const [managerBranch] = useState('Jabal Amman');
  ...
  const filteredData = employees.filter((employee) => {
    ...
    const matchesBranch = employee.branch === managerBranch;
    return matchesSearch && matchesRole && matchesBranch;
  });

  const handleAddEmployee = (newEmployee: Omit<Employee, 'id' | 'savedBy'>) => {
    const newId = employees.length > 0 ? Math.max(...employees.map((e) => e.id)) + 1 : 1;
    setEmployees([...employees, { id: newId, ...newEmployee, branch: managerBranch, savedBy: 'Department Manager' }]);
    closeAddModal();
  };
  ...
  const handleDeleteEmployee = () => {
    if (selectedEmployee && confirm('Are you sure you want to delete this employee? This action requires admin approval.')) {
          ...
          <Box style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
            <Box>
              ...
              <Text size="sm" c="dimmed" style={{ textTransform: 'uppercase' }}>
                Total Employees in {managerBranch}: {filteredData.length}
              </Text>
            </Box>
            <Box>
              <EmployeesActions
